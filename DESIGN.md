...
| U4 | **PostToolUseFailure / PostCompact** availability across versions (verified ≥2.1.141; user runs 2.1.173). PARTIALLY RESOLVED 2026-09: PostCompact *fires*, but its output may carry NO `hookSpecificOutput` — the schema rejects `hookEventName: "PostCompact"` — so the notes re-seed is relayed via `state.compactRecoveryPending` through the next PostToolUse/UserPromptSubmit hook | nudges/cache-reset degrade | failure-streak detection also reads exit markers in PostToolUse output; stale dedup cache self-expires (TTL 10 min) |
